...
def find_meals_and_prices(html):
    ...
    rows = [(entry.find('td', 'meals__column-title'),
             entry.find_all('td', 'meals__column-price')) for entry in rows]
...
def get_request(dateIn):
    """date in format: 2021-08-26"""
    url = f"https://mobil.itmc.tu-dortmund.de/canteen-menu/v3/canteens/341/{dateIn}"
...
def beautify_mensa_plan_vegie(plan_dict):
    ...
    complete_list = []
    for entry in plan_dict:
        if (not bool(entry['type'])) or (('V' in entry['type']) or (('N' in entry['type']))):
            prices = entry['price']
            complete_list.append(
                [f"S: {prices['student']} B: {prices['staff']} G: {prices['guest']}",
                 entry['title']['de']])
...
def plan_for_date(dateIn: datetime.date):
    ...
    try:
        ...
        print('Mensa date for {date_weekday} {date_str} could not be printed.')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--date", dest="date",
                        help="get mensa plan for ATE in the form yyyy-mm-dd", metavar="DATE")
    parser.add_argument("-p", "--plus", dest="plus", type=int,
                        help="get mensa plan for today plus X days",
                        metavar="X")
    parser.add_argument("-n", "--next", dest="next", type=int,
                        help="get mensa lan for next X days including today", metavar="X")
    parser.add_argument("-w", "--week", dest="week", action="store_true",
                        default=False, help="Show plan for whole week.")
    parser.add_argument("-!s", "--!schwein", default=False,
                        help="only show meals not containing pork.", action='store_true')
    parser.add_argument("-v", "--vegi", default=False,
                        help="Only show vegetarian menus.", action='store_true')
    parser.add_argument("-v+", "--vegan", default=False,
                        help="only show vegan menus", action='store_true')
